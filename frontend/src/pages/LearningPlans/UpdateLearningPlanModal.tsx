...
import { UpdateLearningPlanRequest } from "../../types/learing-plan-types";

interface UpdateLearningPlanModalProps {
  planId: string;
  onClose: () => void;
  onSuccess: () => void;
}

const UpdateLearningPlanModal: React.FC<UpdateLearningPlanModalProps> = ({
  planId,
  onClose,
  onSuccess,
}) => {
  const [formData, setFormData] = useState<UpdateLearningPlanRequest>({
    title: "",
    topics: [""],
    resources: [""],
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        const plan = await learningPlanApi.getLearningPlanById(planId);
        setFormData({
          title: plan.title,
          topics: plan.topics,
  ...
  const validateForm = (): boolean => {
    ...
    if (!titleRegex.test(formData.title)) {
      setError(
        "Title must be 3-100 characters long and can only contain letters, numbers, spaces, hyphens, and underscores"
      );
      return false;
    }
    ...
    if (
      formData.timeline.some(
        (t) => !t.topic.trim() || !t.expectedCompletionDate
      )
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await learningPlanApi.updateLearningPlan(currentUser!, planId, formData); // Replace with actual user ID
      onSuccess();
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to update learning plan"
      );
    } finally {
      setIsLoading(false);
    }
...
export default UpdateLearningPlanModal;
